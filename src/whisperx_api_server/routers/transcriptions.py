...
def _raise_for_transcription_error(request_id: str, e: Exception, kind: str) -> None:
    ...
    if isinstance(e, asyncio.TimeoutError) or isinstance(e, TimeoutError):
        logger.warning("Request ID: %s - Timeout: %s", request_id, e)
        raise HTTPException(
            status_code=status.HTTP_504_GATEWAY_TIMEOUT, detail=str(e)
        ) from e
    if isinstance(e, _CUDA_OOM_EXC) and "out of memory" in str(e).lower():
        logger.error("Request ID: %s - CUDA OOM: %s", request_id, e)
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="Out of GPU memory; retry shortly.",
        ) from e
    logger.exception("Request ID: %s - %s failed: %s", request_id, kind, e)
    raise HTTPException(
...
async def transcribe_audio(
    ...
    logger.info(
        "Request ID: %s - Received request to transcribe %s with parameters:         model: %s,         language: %s,         prompt: %s,         response_format: %s,         temperature: %s,         timestamp_granularities: %s,         stream: %s,         hotwords: %s,         suppress_numerals: %s,         highlight_words: %s,         align: %s,         diarize: %s,         speaker_embeddings: %s,         chunk_size: %s,         batch_size: %s",
        request_id,
        source_filename,
        model_name,
        language,
    ...
    try:
        ...
        logger.info("Request ID: %s - Client disconnected; cancelling", request_id)
        raise
    ...
    logger.info(
        "Request ID: %s - Transcription process took %.2f seconds",
        request_id,
        total_time,
    )
...
async def translate_audio(
    ...
    logger.info(
        "Request ID: %s - Received request to translate %s with parameters:         model: %s,         prompt: %s,         response_format: %s,         temperature: %s,         chunk_size: %s,         batch_size: %s",
        request_id,
        source_filename,
        model_name,
        prompt,
    ...
    total_time = time.time() - start_time
    logger.info(
        "Request ID: %s - Translation process took %.2f seconds", request_id, total_time
    )
